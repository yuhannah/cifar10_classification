...
def train():
    ...
    hist = model.fit_generator(train_datagen.flow(x_train, y_train, batch_size=batch_size), steps_per_epoch=8000,
                               epochs=epochs_num, validation_data=(x_test, y_test), shuffle=True)
    # hist = model.fit(x_train, y_train, batch_size=batch_size, epochs=epochs_num, validation_data=(x_test, y_test))

    model.save('./model/deep_CNN_augmentation/cifar10_model.hdf5')
    model.save_weights('./model/deep_CNN_augmentation/cifar10_model_weight.hdf5')
    ...
    plt.savefig("output/deep_CNN_augmentation_accuracy_loss.png")
