...
def train():
    ...
    train_datagen = ImageDataGenerator(rescale=1. / 255, rotation_range=15, width_shift_range=0.15,
    ...
    hist = model.fit_generator(train_datagen.flow(x_train, y_train, batch_size=batch_size),
                               steps_per_epoch=x_train.shape[0] // batch_size, epochs=epochs_num,
                               validation_data=(x_test, y_test), shuffle=True)
    # hist = model.fit(x_train, y_train, batch_size=64, epochs=epochs_num, validation_data=(x_test, y_test), shuffle=True)

    model.save('./model/simple_CNN_augmentation/cifar10_model.hdf5')
    model.save_weights('./model/simple_CNN_augmentation/cifar10_model_weight.hdf5')
    ...
    plt.savefig("output/simple_CNN_augmentation_accuracy_loss.png")
